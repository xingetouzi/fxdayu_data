...
def template(root, **dirs):
    import os

    dirs.setdefault("D", "Stock_D.bcolz")
    _bonus = bonus(os.path.join(root, dirs.pop("bonus", "bonus.bcolz")))
    _factor = factor(os.path.join(root, dirs.pop("factor", "factor.bcolz")))
    _info = info(os.path.join(root, dirs.pop("info", "info")))
    _candle = candle(_bonus, **{key: os.path.join(root, value) for key, value in dirs.items()})
